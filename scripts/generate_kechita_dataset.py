...
Run: python3 scripts/generate_kechita_dataset.py
"""
...
# -------- CONFIG --------
OUT_DIR = "data/seed"
NUM_CUSTOMERS = 60_000      # 6-month dataset (half of original 120k)
NUM_LOANS = 75_000          # 6-month dataset (half of original 150k)
REPAY_SAMPLE_FRAC = 0.40    # fraction of loans to simulate daily repayments for
START_DATE = datetime(2024,11,1)
END_DATE = datetime(2025,4,30)  # 6 months instead of 12
SEED = 42
# ------------------------
...
print("🚀 Starting Kechita Microfinance Dataset Generation...")
print(f"📊 Configuration: {NUM_CUSTOMERS:,} customers, {NUM_LOANS:,} loans")
...
business_types = np.random.choice(["MamaMboga","GroceryShop","Salon","BodaBoda","GeneralShop","Kiosk","WholesaleVendor","Tailor","Barber","MobileShop","Mitumba","ChaiVendor"], size=num_customers, p=[0.18,0.18,0.08,0.12,0.15,0.06,0.05,0.03,0.03,0.06,0.04,0.02])
...
print(f"✅ Generated {target_loans:,} loans")

# Repayments (sampled)
print("⏳ Generating repayments (this may take a moment)...")
...
print("⏳ Generating daily branch performance...")
...
print(f"✅ Generated {len(daily_branch_performance):,} daily performance records")
...
print(f"✅ Generated {len(ai_ready):,} AI customer features")
...
# Generate officer performance based on actual loan data
print("⏳ Generating officer performance metrics...")
officer_loan_data = loans.groupby('officer_id').agg({
    'loan_id': 'count',
    'amount': 'sum'
}).rename(columns={'loan_id': 'loans_handled', 'amount': 'total_disbursed'}).reset_index()

officer_perf_records = []
for _, officer_data in officer_loan_data.iterrows():
    officer_info = officers[officers.officer_id == officer_data.officer_id].iloc[0]
    # Generate monthly records for the past 12 months
    for month_num in range(1, 13):
        month_str = f"2024-{str(month_num).zfill(2)}"
        loans_this_month = int(officer_data.loans_handled / 12 * np.random.uniform(0.7, 1.3))
        approvals = int(loans_this_month * np.random.uniform(0.85, 1.0))
        arrears_recovered = round(np.random.uniform(50000, 500000), 2)
        fraud_hits = np.random.choice([0, 1, 2], p=[0.9, 0.08, 0.02])
        
        officer_perf_records.append({
            "officer_id": officer_data.officer_id,
            "branch_id": officer_info.branch_id,
            "month": month_str,
            "loans_handled": loans_this_month,
            "approvals": approvals,
            "arrears_recovered_ksh": arrears_recovered,
            "fraud_hits": fraud_hits
        })

officer_performance = pd.DataFrame(officer_perf_records)
officer_performance.to_csv(os.path.join(OUT_DIR, "officer_performance.csv"), index=False)
print(f"✅ Generated {len(officer_performance):,} officer performance records")
